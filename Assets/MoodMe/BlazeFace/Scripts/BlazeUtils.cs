...
namespace MoodMe
{
    public static class BlazeUtils
    {
        ...
        static Unity.InferenceEngine.FunctionalTensor ScoreFiltering(Unity.InferenceEngine.FunctionalTensor rawScores, float scoreThreshold)
        {
            return Unity.InferenceEngine.Functional.Sigmoid(Unity.InferenceEngine.Functional.Clamp(rawScores, -scoreThreshold, scoreThreshold));
        }

        public static (Unity.InferenceEngine.FunctionalTensor, Unity.InferenceEngine.FunctionalTensor, Unity.InferenceEngine.FunctionalTensor) NMSFiltering(Unity.InferenceEngine.FunctionalTensor rawBoxes, Unity.InferenceEngine.FunctionalTensor rawScores, Unity.InferenceEngine.FunctionalTensor anchors, float inputSize, float iouThreshold, float scoreThreshold)
        {
            ...
            var nmsBoxes = Unity.InferenceEngine.Functional.Stack(new[]
            {
            yCenter - heightHalf,
            xCenter - widthHalf,
            yCenter + heightHalf,
            ...
            var nmsScores = Unity.InferenceEngine.Functional.Squeeze(ScoreFiltering(rawScores, 100f));
            var selectedIndices = Unity.InferenceEngine.Functional.NMS(nmsBoxes, nmsScores, iouThreshold, scoreThreshold); // (N);

            var selectedBoxes = Unity.InferenceEngine.Functional.Unsqueeze(Unity.InferenceEngine.Functional.IndexSelect(rawBoxes, 1, selectedIndices), 0); // (1, N, 16)
            var selectedScores = Unity.InferenceEngine.Functional.Unsqueeze(Unity.InferenceEngine.Functional.IndexSelect(rawScores, 1, selectedIndices), 0); // (1, N, 1)
        ...
        public static void SampleImageAffine(Texture srcTexture, Unity.InferenceEngine.Tensor<float> dstTensor, float2x3 M)
        {
            var tensorData = Unity.InferenceEngine.ComputeTensorData.Pin(dstTensor, false);
