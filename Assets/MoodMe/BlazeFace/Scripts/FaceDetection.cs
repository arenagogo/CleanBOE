...
namespace MoodMe
{
    public class FaceDetection : MonoBehaviour
    {
        ...
        [Header("Model Assets")]
        [SerializeField] public Unity.InferenceEngine.ModelAsset faceDetector;
        ...
        private Unity.InferenceEngine.Worker m_FaceDetectorWorker;
        private Unity.InferenceEngine.Tensor<float> m_DetectorInput;
        ...
        public bool Process;
        public static Color32[] OutputCropx;
        ...
        public async void Start()
        {
            ...
            var faceDetectorModel = Unity.InferenceEngine.ModelLoader.Load(faceDetector);
            ...
            var outputs = Unity.InferenceEngine.Functional.Forward(faceDetectorModel, 2 * input - 1);
            var boxes = outputs[0];
            ...
            var anchors = Unity.InferenceEngine.Functional.Constant(new Unity.InferenceEngine.TensorShape(k_NumAnchors, 4), anchorsData);
            ...
            m_FaceDetectorWorker = new Unity.InferenceEngine.Worker(faceDetectorModel, Unity.InferenceEngine.BackendType.GPUCompute);

            // Initialize the detector input tensor
            m_DetectorInput = new Unity.InferenceEngine.Tensor<float>(new Unity.InferenceEngine.TensorShape(1, detectorInputSize, detectorInputSize, 3));
...
#if UNITY_2023_2_OR_NEWER
            using var outputIndices = await (m_FaceDetectorWorker.PeekOutput(0) as Unity.InferenceEngine.Tensor<int>).ReadbackAndCloneAsync();
            using var outputScores = await (m_FaceDetectorWorker.PeekOutput(1) as Unity.InferenceEngine.Tensor<float>).ReadbackAndCloneAsync();
            using var outputBoxes = await (m_FaceDetectorWorker.PeekOutput(2) as Unity.InferenceEngine.Tensor<float>).ReadbackAndCloneAsync();
#else
        ...
        void CropAndResizeFace(Texture texture, Vector2 position, Vector2 size)
        {
            ...
            OutputCropx = resizedTexture.GetPixels32();
            croppedFaceData = OutputCropx;
